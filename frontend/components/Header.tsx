...
const Header: React.FC<HeaderProps> = ({ profile, apiBase, apiStatus, onApiChange }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#FDFCF8]/90 backdrop-blur-md border-b border-teal-600/10 px-4 md:px-6 py-4 flex flex-col gap-4">
      <div className="flex items-start md:items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-10 h-10 bg-teal-600 rounded-xl flex items-center justify-center shadow-lg shadow-teal-600/20 shrink-0">
            <svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
            </svg>
          </div>
          <div className="min-w-0">
            <h1 className="text-base md:text-xl font-bold text-slate-900 leading-none truncate">PixelHolo Voice Studio</h1>
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3">
        ...
        <input
          ...
          className="flex-1 bg-white border border-slate-200 rounded-lg px-4 py-2 text-sm md:text-base font-semibold"
        />
        <span className="text-[10px] text-slate-400">Example: http://192.168.x.x:8000</span>
      </div>
    </header>
  );
};
