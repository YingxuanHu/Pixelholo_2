...
const mapError = (error: unknown) => {
  if (error instanceof DOMException) {
    if (error.name === 'NotAllowedError' || error.name === 'SecurityError') {
      return 'Microphone permission was blocked. On iPhone, use HTTPS and allow microphone access.';
    }
...
export const useMobileVoiceInput = ({ apiBase, onFinalText, language = 'en' }: MobileVoiceInputOptions) => {
  ...
  const startRecording = useCallback(async () => {
    if (!hasSupport || isRecording || isTranscribing) return;
    setError(null);
    chunksRef.current = [];
    ...
    try {
      ...
      // Approximate "tap-to-talk" by auto-ending after sustained silence.
  ...
  }, [cleanupAll, cleanupAnalyser, cleanupStream, hasSupport, isRecording, isTranscribing, stopRecording, transcribeBlob]);
